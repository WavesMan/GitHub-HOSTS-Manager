...
def execute_commands(commands):
    ...
    for cmd in commands:
        try:
            ...
            if any(
                keyword in result.stdout.lower() or keyword in result.stderr.lower()
    ...
    try:
        ...
        for entry in json_data:
            try:
                ...
                # 验证IP地址格式
                if not all(
                    part.isdigit() and 0 <= int(part) <= 255 for part in ip.split(".")
                ):
...
def open_github():
    """打开 GitHub 链接"""
    webbrowser.open("https://github.com/WavesMan/GitHub-HOSTS-Manager")
...
def get_latest_version():
    ...
    try:
        response = requests.get(
            "https://api.github.com/repos/WavesMan/GitHub-HOSTS-Manager/releases/latest"
        )
        if response.status_code == 200:
...
def export_logs():
    ...
    save_path = filedialog.asksaveasfilename(
        defaultextension=".log", filetypes=[("日志文件", "*.log")], title="保存日志文件"
    )
...
def create_gui():
    ...
    # GitHub520 管理
    github520_label = ttk.Label(
        tools_frame, text="GitHub DNS HOSTS 修改器", font=("Microsoft YaHei", 14)
    )
    ...
    remove_github520_button = ttk.Button(
        ...
        text="移除 GitHuB HOST",
        style="TButton",
        command=lambda: remove_github520_hosts()
        and messagebox.showinfo("成功", "已成功移除 GitHub520 hosts"),
    )
    ...
    repo_link_label = tk.Label(
        repo_frame,
        text="https://github.com/WavesMan/GitHub-HOSTS-Manager",
        font=("Microsoft YaHei", 10),
        bg="white",
        cursor="hand2",
        anchor="w",
    ...
    latest_version_label.bind(
        "<Enter>", lambda event: on_enter(event, latest_version_label)
    )
    latest_version_label.bind(
        "<Leave>", lambda event: on_leave(event, latest_version_label)
    )
